...
class LeaveRequestController extends Controller
{
    ...
    {
        $leaveRequests = LeaveRequest::where('user_id', Auth::id())->latest('created_at')->get();
        $leaveBalnace  = LeaveBalance::where('user_id', Auth::id())->first();
        return view('leave_requests.index', compact('leaveRequests', 'leaveBalnace'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($requestedDays > $leaveBalance->total_leave_days - $leaveBalance->used_leave_days) {
            return back()->with('error', 'You have exceeded your leave days limit. Please apply for unpaid leave.');
        }
        ...
        // Gửi email thông báo
        Mail::to($user->email)->send(new LeaveRequestSubmitted());
