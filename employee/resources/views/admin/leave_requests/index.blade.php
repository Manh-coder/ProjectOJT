                    ...
                    <tr class="border-b hover:bg-gray-100">
                        ...
                        <td class="px-4 py-2 text-sm text-gray-700">
                            @if($leaveRequest->status == 'pending')
                                <form action="{{ route('admin.leave_requests.update', $leaveRequest->id) }}" method="POST">
                                    @csrf
                                    @method('PUT')
                                    <button type="submit" name="action" value="approved" class="text-green-500">Approve</button>
