...
@section('content')
    ...
    <div class="form-container">
        ...
        <a href="{{ route('employees.export', ['per_page' => 1]) }}" class="button-sm"
            style="background-color: #28a745; color: #fff;">Export</a>
    </div>
            ...
            @else
                @foreach ($employees as $key => $employee)
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('employees.show', $employee->id) }}" class="button-sm"
                                style="background-color: #007bff; color: #fff;">Details</a>
                            <a href="{{ route('employees.edit', $employee->id) }}" class="button-sm"
                                style="background-color: #ffc107; color: #000;">Edit</a>
                            ...
                            <form action="{{ route('employees.destroy', $employee->id) }}" method="POST"
                                ...
                                <button type="submit" class="button-sm"
                                    style="background-color: #dc3545; color: #fff;">Delete</button>
                            </form>
                        </td>
                    </tr>
                @endforeach
